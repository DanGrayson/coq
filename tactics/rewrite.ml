...
let apply_lemma flags (evm,c) left2right by loccs : strategy =
  fun () env avoid t ty cstr evars ->
    let hypinfo = decompose_applied_relation env
      (goalevars evars) evm flags None c left2right
    in
	  ...
	  in
		...
		| Some (Some r) ->
		    ...
		    in 
		      match prf with
		      | RewPrf (rel, prf) ->
			state, Some (Some (apply_constraint env avoid res.rew_car
				      rel prf (prop,cstr) res))
		      | _ -> state, Some (Some res)
	  ...
	  | x ->
		...
		| Some r ->
		  let ctxc = mkCase (ci, lift 1 p, lift 1 c, Array.of_list (List.rev (brs' x))) in
		    state, Some (Some (make_leibniz_proof ctxc ty r))
		| None -> state, x
	    else
	...
	in 
	  ...
	  | Some (Some r) ->  
	    let rel, prf = get_rew_prf r in
	      state, Some (Some (apply_constraint env avoid r.rew_car rel prf (prop,cstr) r))
	  | x -> state, x)
      | _ -> state, None
  in aux
...
let transitivity state env avoid prop (res : rewrite_result_info) 
    (next : 'a pure_strategy) : 'a * rewrite_result option =
  let state, res' = 
    next state env avoid res.rew_to res.rew_car (prop, get_opt_rew_rel res.rew_prf) res.rew_evars 
  in
  state, match res' with
  | None -> None
...
module Strategies =
  struct
    ...
    let refl : 'a pure_strategy =
      fun s env avoid t ty (prop,cstr) evars ->
    ...
    let seq (first : 'a pure_strategy) (snd : 'a pure_strategy) : 'a pure_strategy =
      fun state env avoid t ty cstr evars ->
...
let rewrite_with flags c left2right loccs : strategy =
  fun () env avoid t ty cstr evars ->
    ...
    let _, res = 
      rewrite_strat default_flags loccs (hypinfo,0) env avoid t ty cstr (gevars, cstrevars evars)
    in (), res
  ...
  | StratTerms l -> 
    (fun state env avoid t ty cstr evars ->
     let l' = interp_glob_constr_list env (goalevars evars) (List.map fst l) in
       Strategies.lemmas rewrite_unif_flags l' state env avoid t ty cstr evars)
  | StratEval r -> 
    (fun state env avoid t ty cstr evars ->
     let (sigma,r_interp) = Tacinterp.interp_redexp env (goalevars evars) r in
       Strategies.reduce r_interp state env avoid t ty cstr (sigma,cstrevars evars))

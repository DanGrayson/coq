...
let catch_error call_trace tac g =
  ...
  | Ploc.Exc (_,LtacLocated _) as e -> raise e
  | e ->
    let (nrep,loc',c),tail = list_sep_last call_trace in
    let loc,e' = match e with Ploc.Exc(loc,e) -> loc,e | _ ->dloc,e in
    if tail = [] then
      let loc = if loc = dloc then loc' else loc in
      raise (Ploc.Exc(loc,e'))
    else
      raise (Ploc.Exc(loc',LtacLocated((nrep,c,tail,loc),e')))
...
and eval_with_fail ist is_lazy goal tac =
  try
    ...
    | a -> a)
  with
    | FailError (0,s) | Ploc.Exc(_, FailError (0,s))
    | Ploc.Exc(_,LtacLocated (_,FailError (0,s))) ->
	raise (Eval_fail (Lazy.force s))
    | FailError (lvl,s) -> raise (FailError (lvl - 1, s))
    | Ploc.Exc(s,FailError (lvl,s')) ->
	raise (Ploc.Exc(s,FailError (lvl - 1, s')))
    | Ploc.Exc(s,LtacLocated (s'',FailError (lvl,s'))) ->
	raise (Ploc.Exc(s,LtacLocated (s'',FailError (lvl - 1, s'))))

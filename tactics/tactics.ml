...
let (assumption : tactic) = fun gl ->
  ...
  let rec arec only_eq = function
	...
	if (only_eq && eq_constr t concl) 
	  ...
	  in
	    if b then 
	      tclTHEN (tclEVARS evm) (refine_no_check (mkVar id)) gl
...
let letin_tac_gen with_eq name (sigmac,c) test ty occs gl =
  ...
  let sigma,newcl,eq_tac = match with_eq with
	...
	| _ -> error"Expect an introduction pattern naming one hypothesis." in
      ...
      let sigma, eq = Evd.fresh_global (pf_env gl) (project gl) eqdata.eq in
      let sigma, refl = Evd.fresh_global (pf_env gl) sigma eqdata.refl in
      let eq = applist (eq,args) in
      let refl = applist (refl, [t;mkVar id]) in
      sigma, mkNamedLetIn id c t (mkLetIn (Name heq, refl, eq, ccl)),
      tclTHEN
	(intro_gen loc (IntroMustBe heq) lastlhyp true false)
	(thin_body [heq;id])
    | None ->
	project gl, mkNamedLetIn id c t ccl, tclIDTAC in
  tclTHENLIST
    [ tclEVARS sigma; tac; convert_concl_no_check newcl DEFAULTcast;
...
let symmetry_red allowred gl =
  ...
  | Some eq_data,_,_ ->
	...
	(pf_constr_of_global eq_data.sym apply) gl
...
let transitivity_red allowred t gl =
  ...
  | Some eq_data,_,_ ->
	 ...
	 | Some t -> pf_constr_of_global eq_data.trans 
	   (fun trans -> apply_list [trans;t])) gl
  | None,eq,eq_kind ->
      match t with
...
let abstract_subproof id tac gl =
  ...
  let evd, ctx, concl = 
    ...
    let ctx = Evd.get_universe_context_set evd in
      evd, ctx, nf concl
  in
  let _, gk, _, _ = Pfedit.current_proof_statement () in
  let const, subst = Pfedit.build_constant_by_tactic id ~goal_kind:gk secsign 
    (concl, ctx)
  ...
  let gl = { gl with sigma = Evd.emit_side_effects effs evd; } in
  exact_no_check

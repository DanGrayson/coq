...
let cons_expl x p = lazy (x :: Lazy.force p)
  ...
  | (arc,p)::lt_todo, le_todo ->
    if List.memq arc lt_done then
      ...
      | [] ->
        ...
        | u :: lt ->
          let arc = repr g u in
          let p = cons_expl (Lt, Universe.make u) p in
          if arc == arcv then
      ...
      | u :: le ->
        let arc = repr g u in
        let p = cons_expl (Le, Universe.make u) p in
        if arc == arcv then
          if strict then LT p else LE p
        else find ((arc, p) :: lt_todo) lt le
      in
    ...
    if arc == arcv then
	 ...
	 come later from [le_todo]. *)
      ...
      if (List.memq arc lt_done) || (List.memq arc le_done) then
        ...
        | [] ->
          let fold accu u =
            let node = (repr g u, cons_expl (Le, Universe.make u) p) in
            node :: accu
          in

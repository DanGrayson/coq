...
let make_bl_scheme mind =
  ...
  let ctx = Univ.ContextSet.empty (*FIXME univs *) in
    ([|
      fst (Pfedit.build_by_tactic (Global.env()) (bl_goal,ctx)
	     (compute_bl_tact (!bl_scheme_kind_aux()) (ind,Univ.Instance.empty) lnamesparrec nparrec))|], Evd.empty_evar_universe_context), eff
...
let make_lb_scheme mind =
  ...
  let lb_goal, eff = compute_lb_goal ind lnamesparrec nparrec in
    ([|fst (Pfedit.build_by_tactic (Global.env())
	 (lb_goal, Univ.ContextSet.empty)
	 (compute_lb_tact (!lb_scheme_kind_aux()) ind lnamesparrec nparrec))|],
     Evd.empty_evar_universe_context (* FIXME *)), eff

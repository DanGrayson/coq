...
let declare_fix (_,poly,_ as kind) ctx f (def,seff) t imps =
  let ce = definition_entry ~types:t ~poly ~univs:ctx ~seff def in
...
let build_wellfounded (recname,n,bl,arityc,body) r measure notation =
  ...
  let ctx = Evd.get_universe_context_set !evdref in
    ignore(Obligations.add_definition
      recname ~term:evars_def evars_typ ctx evars ~hook)
